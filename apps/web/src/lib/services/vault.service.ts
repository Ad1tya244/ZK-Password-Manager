...
export const createVaultItem = async (
    ...
    return await prisma.vault.create({
        data: { userId, encryptedBlob: data, iv, authTag },
    });
};
...
export const getVaultItem = async (userId: string, id: string) => {
    return await prisma.vault.findFirst({ where: { id, userId } });
};
...
export const updateVaultItem = async (
    ...
    return await prisma.vault.update({
        where: { id },
        data: { encryptedBlob: data, iv, authTag },
    });
};
...
export const deleteVaultItem = async (userId: string, id: string) => {
    const existing = await prisma.vault.findFirst({ where: { id, userId } });
    if (!existing) throw new Error("Item not found");
    return await prisma.vault.delete({ where: { id } });
};
