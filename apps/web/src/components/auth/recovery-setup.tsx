...
export default function RecoverySetup({ onClose, isRotation = false }: { onClose: () => void, isRotation?: boolean }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`bg-zinc-900 border ${isRotation ? "border-amber-500/30" : "border-zinc-700"} rounded-2xl p-6 w-full max-w-lg shadow-2xl relative overflow-hidden`}>
                <div className={`absolute top-0 left-0 w-full h-1 ${isRotation ? "bg-gradient-to-r from-amber-500 to-orange-500" : "bg-gradient-to-r from-emerald-500 to-cyan-500"}`}></div>
                ...
                {step === "initial" && (
                    <div className="space-y-4">
                        ...
                        <div className={`p-4 rounded-lg ${isRotation ? "bg-red-500/10 border border-red-500/20" : "bg-amber-500/10 border border-amber-500/20"}`}>
                            <p className={`${isRotation ? "text-red-400" : "text-amber-200"} text-xs font-semibold uppercase tracking-wide mb-1`}>
                                {isRotation ? "Critical Warning" : "Warning"}
                            </p>
                            <p className={`${isRotation ? "text-red-300" : "text-amber-100/80"} text-sm leading-relaxed`}>
                                {isRotation
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="bg-zinc-950 border border-zinc-800 p-4 rounded-xl font-mono text-center break-all relative group">
                            ...
                            <button
                                ...
                                className="absolute top-2 right-2 p-2 bg-zinc-800 text-zinc-400 rounded hover:text-white hover:bg-zinc-700 transition-all opacity-0 group-hover:opacity-100"
                                title="Copy"
                            >
                        ...
                        <div className="flex justify-end gap-3">
                            <button
                                ...
                                className="w-full px-6 py-3 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white rounded-lg font-medium transition-colors"
                            >
