...
## Elevator Pitch
The Zero-Knowledge Password Manager is a security-first, monorepo web application that implements local client-side AES-GCM 256-bit vault encryption, Argon2id master password hashing on the backend, and TOTP-based multi-factor authentication. Built using Next.js 14 and Express.js, the system enforces a strict threat model where the server is treated as completely untrusted, ensuring that plaintext credentials can never be accessed, decrypted, or compromised even in the event of a full database breach.
...
Supports RFC 6238 Time-based One-Time Passwords (TOTP). During setup, a QR code is generated via `qrcode` and scanned using apps like Google Authenticator. Verification is performed using the `otplib` package on the server before issuing session tokens.

### 6. Double Verification and Rate Limiting
Critical endpoints like Account Deletion require double verification: both the Master Password and the TOTP token must be validated. Authentication routes are secured with IP-based rate limiting (10 attempts per minute), and the server implements a 10-minute database lockout after 5 failed login attempts.
...
+-------------------------------------------------+-------------------------------------------------+
                                                  |
                                                  | HTTPS API Calls (Axios with credentials)
                                                  v
+---------------------------------------------------------------------------------------------------+
|                                      EXPRESS BACKEND (apps/api)                                   |
|                                                                                                   |
|  +---------------------------------------------------------------------------------------------+  |
|  | Routers: /auth & /vault (routes/auth.routes.ts & routes/vault.routes.ts)                     |  |
|  +----------------------------------------------+----------------------------------------------+  |
|                                                 |                                                 |
|                                                 v                                                 |
|  +---------------------------------------------------------------------------------------------+  |
|  | Middlewares (authenticateToken / loginLimiter rate-limiting)                                |  |
|  +----------------------------------------------+----------------------------------------------+  |
|                                                 |                                                 |
|                                                 v                                                 |
|  +---------------------------------------------------------------------------------------------+  |
|  | Controllers (auth.controller.ts & vault.controller.ts)                                      |  |
|  +----------------------------------------------+----------------------------------------------+  |
|                                                 |                                                 |
|                                                 v                                                 |
|  +---------------------------------------------------------------------------------------------+  |
|  | Services Layer (auth.service.ts & vault.service.ts)                                         |  |
|  |                                                                                             |  |
|  |   +--------------------------------------+       +---------------------------------------+  |  |
...
*   **`apps/web`:** Next.js 14 frontend handling UI components, page layouts, local encryption, decryption, and API communication.
    ...
    *   [VaultDashboard](file:///Users/adityadivakar/Documents/Projects/zk-password-manager/apps/web/src/components/vault-dashboard.tsx): Manages items, account settings, recovery setup, and password utilities.
    *   [encryption.utils.ts](file:///Users/adityadivakar/Documents/Projects/zk-password-manager/apps/web/src/utils/encryption.utils.ts): High-level wrapper containing recovery key derivation, session state initialization, wrapping, and unwrapping.
*   **`apps/api`:** Express.js REST server.
    *   [auth.controller.ts](file:///Users/adityadivakar/Documents/Projects/zk-password-manager/apps/api/src/controllers/auth.controller.ts): Routes and coordinates requests for login, registration, 2FA setup, recovery initiation, and reset.
    *   [vault.controller.ts](file:///Users/adityadivakar/Documents/Projects/zk-password-manager/apps/api/src/controllers/vault.controller.ts): Handles CRUD endpoints for encrypted vault records.
    *   [auth.service.ts](file:///Users/adityadivakar/Documents/Projects/zk-password-manager/apps/api/src/services/auth.service.ts): Implements credentials verification, Argon2 hashing, database lockout checking, and user deletion.
    *   [vault.service.ts](file:///Users/adityadivakar/Documents/Projects/zk-password-manager/apps/api/src/services/vault.service.ts): Communicates with Prisma to perform database operations on vault items.
*   **`packages/crypto`:**
    *   [client.ts](file:///Users/adityadivakar/Documents/Projects/zk-password-manager/packages/crypto/src/client.ts): Client-side Web Crypto API wrappers for PBKDF2 derivation, AES-GCM encryption/decryption, and base64 helper utilities.
...
sequenceDiagram
    ...
    Browser->>API: POST /auth/register (username, password)
    ...
    Browser->>API: POST /auth/vek (encryptedVEK, vekIV, vekAuthTag)
...
sequenceDiagram
    ...
    alt 2FA is Enabled
        ...
        Browser->>API: POST /auth/verify-2fa (username, token)
...
sequenceDiagram
    ...
    Browser->>API: POST /vault (encryptedBlob, iv, authTag)
...
sequenceDiagram
    ...
    Browser->>API: POST /auth/recovery/init (recoveryKeyHash)
    ...
    Browser->>API: POST /auth/recovery/reset (recoveryKeyHash, newPassword, newWrappedVEK, newVaultSalt...)
    API->>API: Hash newPassword (Argon2id)
    API->>DB: Update User record (new passwordHash, new vaultSalt, new encryptedVEK, clear lockout)
...
| **Next.js 14** | Frontend Framework | Selected for its modern App Router architecture, responsive layout rendering, and React 18 integration. |
| **Express.js** | Backend Web Framework | Lightweight, modular framework that facilitates custom authentication routing and rate-limiting middleware. |
...
## My Contributions
*   **Cryptographic Architecture Design:** Designed and developed the dual-key client-side cryptographic system (PBKDF2-HMAC-SHA256 and AES-GCM 256-bit) using browser-native APIs.
*   **Zero-Knowledge Recovery System:** Engineered the recovery system using HKDF-SHA256 to derive independent decryption keys (Recovery KEK) and secure lookup indices (`recoveryKeyHash`), ensuring no plaintext recovery keys are ever transmitted.
*   **Monorepo Setup:** Configured the TurboRepo workspace, split shared dependencies, and compiled the shared `@zk/crypto` package for use by the backend and frontend apps.
*   **Database Schema & Prisma Migrations:** Created the Prisma model structures to handle base64-encoded fields, salt distribution, failed attempts tracking, and cascading deletion linkages between users and vaults.
*   **Frontend Security State Management:** Built the React session management singleton (`EncryptionService`) ensuring keys are loaded as non-extractable `CryptoKey` objects and destroyed upon logout or session timeouts.
*   **Backend Hardening:** Coded the API rate limit middleware, HTTP cookie handlers, and database lockout routines protecting the login endpoints from distributed brute-force attempts.
*   **Automated Verification Suites:** Programmed end-to-end integration tests (`test-recovery.ts`) that simulate the full signup, key wrapping, key rotation, and account recovery cycles.
...
*   The API blocks traffic from IPs exceeding 10 requests per minute on authentication endpoints.
...
During setup, the client generates a 256-bit recovery key. Using HKDF-SHA256, it derives a Recovery KEK and a recovery lookup hash. The client encrypts the VEK with the Recovery KEK and uploads it along with the lookup hash. If the master password is lost, the recovery key derives the Recovery KEK, retrieves the recovery-wrapped VEK, decrypts it, and re-wraps it with a new master KEK derived from a new password.
...
Deriving a KEK using PBKDF2 with 100,000 iterations takes approximately 120ms to 180ms on standard client devices. This delay is imperceptible to users during login but provides a significant barrier against offline cracking attempts.
...
TurboRepo provides a monorepo structure that allows sharing the `@zk/crypto` library and `@zk/database` package between the Next.js frontend and Express backend. It optimizes build, test, and lint pipelines by caching artifacts, reducing development iteration times.

### 20. What security measures are taken to secure the REST API against unauthorized access?
The API secures all vault endpoints using the `authenticateToken` middleware. This middleware intercepts requests, extracts the JWT access token from the cookies, and validates its signature. If the token is missing or invalid, the request is rejected with a 401 or 403 status code.
...
*   **Duration:** 4 Weeks (Intense development and security hardening)
*   **Team Size:** 1 (Solo Developer & Security Engineer)
*   **My Role:** Full-Stack Developer & Security Engineer
*   **Tech Stack:** Next.js 14, React 18, Express.js, TypeScript, Prisma ORM, MySQL, Web Crypto API, Argon2id, JWT, OTPLib, TurboRepo
*   **Key Features:** Zero-Knowledge Key Wrapping Architecture, Client-Side AES-GCM 256-bit Encryption, ZK Account Recovery via HKDF-SHA256, RFC 6238 TOTP Multi-factor Authentication, Argon2id Password Hashing, API Rate Limiting, and Account Lockouts.
